...
def ask_q_login(ask_q, N, user_log, sel_item = 1):
    ...
    if score != 0:
        ...
        # In sage, str.format()에서 f 변환이 지원되지 않는다. 그래서 제거함.
        #print( "Total time is {t}sec for {N}, each time is {t1}sec.".format(t=time_all, N=N, t1=time_each))
        print( "Total time is %.2f sec for %d, each time is %.2f sec." % (time_all, N, time_each))
    else:
        # time_each = time_all
        # In sage, str.format()에서 f 변환이 지원되지 않는다. 그래서 제거함.
        print( "Total time is %.2f sec for %d (each time is not allowed.)." % (time_all, N))
